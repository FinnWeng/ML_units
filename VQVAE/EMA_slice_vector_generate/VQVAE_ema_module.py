...
class VQVAE:
    def __init__(self, embedding_dim, _num_embeddings, commit_loss_coef, scope):
        ...
        self.partition = 8
        self._embedding_dim = int(embedding_dim/self.partition)
    ...
    def VQVAE_layer(self, inputs):
        ...
        print("inputs:", inputs) # shape=(?, 14, 14, 256)
        ...
        with tf.control_dependencies([
            ...
            print("flat_inputs:", flat_inputs) # (b*partion, h*w, 32)
        ...
        self.pixel_wise_embedding_count = tf.reshape(tf.reduce_sum(multi_hot_encodings, axis=0),[tf.shape(inputs)[1],tf.shape(inputs)[2],self._num_embeddings])#  (b,H*W,self._num_embeddings) => (H*W,self._num_embeddings) => (H,W,self._num_embeddings)
