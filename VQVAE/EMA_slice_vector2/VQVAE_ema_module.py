...
class VQVAE:
    ...
    def variable_def(self):
        ...
        with tf.variable_scope(self.scope, 'VQVAE', reuse=tf.AUTO_REUSE):
            ...
            self.embedding_total_count = tf.get_variable("embedding_total_count", [1, self.part_k*self.partition],
    ...
    def loop_assign_moving_avg(self, encodings, sliced_inputs):
        ...
        flat_input = tf.reshape(sliced_inputs,[tf.shape(sliced_inputs)[0]*tf.shape(sliced_inputs)[1],self.partition,self.part_embd_dim])
        print("flat_input:",flat_input)
        flat_encoding = tf.reshape(encodings,[-1,self.part_k*self.partition]) # self.part_k*self.partition != k
        print("flat_encoding::",flat_encoding) 
        embedding_count = tf.reshape(tf.reduce_sum(encodings, axis=0), [1, -1]) 
        ...
        input_contrib_per_embedding_value = tf.matmul(tf.expand_dims(flat_input,-1), tf.expand_dims(encodings,axis=-2))
        ...
        input_contrib_per_embedding_value = input_contrib_per_embedding_value / tf.cast(tf.reshape(self.embedding_total_count,[self.partition,1,self.part_k]),
                                                                                        tf.float32)
        print("input_contrib_per_embedding_value:", input_contrib_per_embedding_value) # (128, 1024),
        input_contrib_per_embedding_value = tf.reshape(input_contrib_per_embedding_value,[self.partition,self.part_k,self.part_embd_dim])
        print("input_contrib_per_embedding_value:", input_contrib_per_embedding_value)

        w_defference = (1 - self.gamma) * (self._w - input_contrib_per_embedding_value) * tf.reshape(update_or_not,[self.partition,self.part_k,1])
    ...
    def quantize(self, encoding_indices):
        ...
        #     print("self._w[i,:,:]:",self._w[i,:,:])
        #     print("trans_idx[i,:,:]:",trans_idx[i,:,:])
        #     embd = tf.nn.embedding_lookup(self._w[i,:,:], trans_idx[i,:,:], validate_indices=False)
        #     print("embd:",embd)
        #     stack.append(embd)
        ...
        # quantize = tf.stack(stack,axis=0)
    ...
    def VQVAE_layer(self, inputs):
        ...
        same_idx =tf.reduce_sum(tf.cast(tf.equal(non_max_encoding_indices,encoding_indices),tf.float32))
        ...
        multi_hot_encodings = tf.one_hot(non_max_encoding_indices,self.part_k)
        multi_hot_encodings = tf.reduce_sum(multi_hot_encodings,axis=-2)
