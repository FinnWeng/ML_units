...
class MODEL:
    ...
    def main(self, img, training_status):
        with tf.variable_scope("vea_autoencoder", reuse=tf.AUTO_REUSE):
            ...
            #     top_VQVAE_instance = VQVAE_module.VQVAE(self.latent_base, self.latent_size, 0.25, "top_VQVAE")
            ...
            l4_raw_output = tf.keras.layers.Conv2DTranspose(self.filter_num * 2, kernel_size=3, strides=2, padding="SAME",
                                                kernel_initializer=self.kernel, activation=tf.nn.tanh)(l3_output)
            ...
            #     bottom_VQVAE_instance = VQVAE_module.VQVAE(self.latent_base * 2, self.latent_size, 0.25,
            #                                                    "bottom_VQVAE")
            ...
            l5_raw_output = tf.keras.layers.Conv2DTranspose(self.filter_num * 1, kernel_size=3, strides=2, padding="SAME",
                                                kernel_initializer=self.kernel, activation=tf.nn.tanh)(l4_output)
    ...
    def loss_function(self):
        ...
        return tf.reduce_mean(self.logpx_z) 
